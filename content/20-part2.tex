% ===================================================================
%  DUESMA PARTO -- leaves 119 and following
%
%  Leaf 119: half title of the second part. A page of apparatus: each
%  line is placed at its MEASURED ORDINATE and takes up no height; no
%  line displaces another.
%
%  Measurements by tools/apparatus.py (leaf 119):
...
%
%  The letterspacings 219 and 261 are not the ones apparatus.py computes
%  (201 and 227): they correct them on the SET TEXT. The rule of
%  apparatus estimates the bare width by a trial composition, and its
%  estimate degrades when the line carries many word spaces -- « PER
%  DERIVO O PER KOMPOZO » counts four, and came out 2,54 mm too short.
%  Once corrected, the page's four widths fall within 0,08, 0,00, 0,08
%  and 0,51 mm of the measurement; the volume's half title, already
%  accepted, is within 0,26, 0,42 and 1,27 mm.
%
%  WEIGHT: the three lines are in ROMAN, VORTIFADO included, which at
%  first sight looks bold from its size. Two measurements refute it.
%  The page's stem/capital ratios -- 0,140, 0,175, 0,160 -- all fall
%  within the range of leaf 5's half title, whose three lines are roman
%  and give 0,152, 0,167 and 0,146: the stem separates nothing, as is
%  recorded in docs/journal.md. And VORTIFADO, brought back to the
%  capital height of « LINGUO INTERNACIONA IDO », is superposable on it
%  -- the same serifs, the same O, the same R. It is the same roman,
%  larger.
%
%  The rule is the same thin rule as that of the notes and that of the
%  end of the part: only the length changes (8,21 mm here).
% ===================================================================

% --- Leaf 119 (folio 115): half title of the second part ------------
\begin{VUpage}[119]{}
...
% --- Leaf 120 (folio 116): blank ------------------------------------
% The line detector finds none on this leaf: a blank verso, like that
% of the volume's half title (leaf 6).
\begin{VUpage}[120]{}
\end{VUpage}

% -------------------------------------------------------------------
%  leaf 121 -- folio 117: NO RUNNING FOLIO in the facsimile.
%  The head of the page is bare (checked under magnification, y 90 to
%  320): it is the first page of the second part, and the printer omits
%  the folio there as is usual. The folio field therefore stays empty.
%  Two centred titles, measured by tools/apparatus.py then corrected on
%  the set text: ELEMENTI DI VORTO. (cap 2,540 mm, 37,08 mm wide) and
%  PROCEDI DI VORTIFADO. (cap 2,455 mm, 42,84 mm).
%  The block of notes carries THREE paragraphs for two notes: the third,
%  « La maxim multa gramatikisti docas... », is indented like the other
%  two but carries no number. Checked under magnification.
% -------------------------------------------------------------------
\begin{VUpage}[121]{}
\VUnotes{117.69mm}{%
...
% -------------------------------------------------------------------
%  leaf 122 -- folio 118.
%  A TITLE BLOCK OF THREE TIERS, the volume's first: a title in
%  semibold capitals (cap 2,455 mm) over TWO subtitles in lower-case
%  italic at the size of the text (cap 1,778 mm, that of the page's
%  lines of body). The four excesses reported by the detection at lines
%  9 to 12 are the whites of that block.
%  The title carries an asterisk as a reference: the footnote opens
%  with the same asterisk and not with a number.
%  IRREGULARITIES OF THE ORIGINAL, kept: a capital « E » without its
%  accent in a French title whose remainder is accented; and « directeur
%  d'ecole » set in roman when the other French segments of the same
%  note are in italic.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 123 -- folio 119. A very long block of notes (31 lines against
%  12 of body): the rule rises to 72,05 mm.
%  A TYPO OF THE ORIGINAL, kept: the word « signifikas » is in italic
%  after « lanjo » and in roman after « linjo », two words apart on the
%  same line.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 124 -- folio 120.
%  TYPOS OF THE ORIGINAL, kept: in note (4), « regulo cd expliko » for
%  « od » (checked under magnification, the letter is a c open on the
%  right); and two opening brackets never closed, one in note (2), the
%  other at the line « (ma on darfas dicar ».
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 125 -- folio 121.
%  THE VOLUME'S FIRST SUBTITLE: under « DERIVADO PER AFIXI. », the line
%  « (Mediata derivado.) » is in lower-case italic, at the size of the
%  running text (cap 21 px, like the page's lines of body, against
%  29 px for the title). \VUtitre would set it in semibold capitals:
%  hence \VUsoustitre, added to the preamble for this case.
%  AN IRREGULARITY OF THE ORIGINAL, kept: on the last line, the French
%  gloss is set in ROMAN, where the book everywhere else italicises
%  what is quoted from another language.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 126 -- folio 122.
%  The line detector did NOT see this page's running folio: the first
%  band of the geometric survey is the title « AFIXI. ». The folio is
%  printed all the same (checked under magnification, baseline at
%  y = 147). The folio field is therefore filled in.
%  AN INVISIBLE LINE RECOVERED: « prefixo. », the second and last line
%  of note (2). The detection announced it as « a clean double, no
%  white »; read under magnification, it is there. The page carries 10
%  lines of note.
%  AN IRREGULARITY OF THE ORIGINAL, kept: « anti- » is in bold in the
%  body and in italic in note (3).
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 127 -- folio 123.
%  TYPOS OF THE ORIGINAL, kept: « ciencal » and « matematikal » for
%  ciencala / matematikala, where the neighbouring « teknikala » keeps
%  its -a; and an opening quotation mark, at the paragraph « auto. »,
...
% -------------------------------------------------------------------
%  leaf 128 -- folio 124.
%  The detection announced a missing last line: it was foxing, checked
%  under magnification.
%  A TYPO OF THE ORIGINAL, kept: « rimarijinta » without the a, where
%  the same word is printed « rimariajinta » two lines above.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 129 -- folio 125.
%  The detection announced a missing last line: foxing, as at leaf 128.
%  Checked under magnification.
%  AN IRREGULARITY OF THE ORIGINAL, kept: in note (2) the italic takes
%  in the linking « e » and runs to the final stop, where notes (1) and
%  (3) leave that same « e » in roman.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 130 -- folio 126.
%  IRREGULARITIES OF THE ORIGINAL, kept: « (segun la \textit{kazo}) »
%  then « (segun la kazo) », italic once and roman the next time, one
%  line apart; and a full stop where the punctuation expected is a
%  comma, in the reference « (Progreso, VII, 65. e VI, 212.) ».
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 131 -- folio 127.
%  IRREGULARITIES OF THE ORIGINAL, kept: « bo- » is in bold at the
%  paragraph « mi-. » and in roman nine lines below; « stifa » is in
%  italic on one line and in roman on the next; and a full stop where
%  the parallel line prints a comma, before « e. c. ».
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 132 -- folio 128.
%  AN INVISIBLE LINE RECOVERED: « IV, 68.) », the second and last line
%  of note (2). The detection announced it as « a clean double, no
%  white »; checked under magnification by the assembler. The block of
%  notes therefore carries 13 lines where the geometry showed only 12.
%  A TYPO OF THE ORIGINAL, kept: « automooilo » for automobilo, in note
%  (4). Checked under magnification at x 10 by the assembler: the
%  disputed letter has NO ink above the x-height, where the ascender of
%  the neighbouring l and the dot of the i print black and full. It is
%  an o, a broken sort or a compositor's slip; we transcribe what is
%  printed.
%  Another irregularity kept: in note (1), « plena » stays in roman
%  where pleno, pleneco, utilo, utileco and utila are italic.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 133 -- folio 129. The first page of a gathering: it carries the
%  SIGNATURE « 5 » under the block of notes, on the left. Ink surveyed
%  from y 1865 to 1890 px, x 103 to 119.
%  TYPOS OF THE ORIGINAL, kept: an opening quotation mark never closed
%  in the quotation of decision 1629; and « 647 et 648 », with the
%  French « et » instead of the Ido « e ».
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 134 -- folio 130. The page OPENS ON A TITLE.
%  TYPOS OF THE ORIGINAL, kept: an opening quotation mark never closed
%  in the last paragraph of the note; and two asterisk references,
%  before « equi-. » and « ko-. », to which no note at the foot of the
%  page answers -- the foot carries only one numbered note.
%  The block of notes counts three paragraphs, two of them unnumbered.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 135 -- folio 131.
%  The two excesses reported by the detection frame the centred title:
%  91,4 px above, 52,6 px below. No line is missing, both zones were
%  enlarged and are blank.
%  IRREGULARITIES OF THE ORIGINAL, kept: « mi- » quoted in ROMAN on one
%  line, where « ne- » and every other prefix quoted on the page is in
%  italic; and an opening bracket that is closed only two sentences
%  later.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 136 -- folio 132. TWO centred titles.
%  THE VERDICT « a clean double, no white » IS AT FAULT HERE, for the
%  first time in the volume. The detection announced a missing line at
%  line 13; the ink profile across the whole width of the page gives
%  ZERO inked pixels from rows 768 to 819 (two or three specks apart).
%  The cause is identified: the white that PRECEDES the title
%  « SUFIXI. » is almost exactly two pitches, which the detection reads
%  as a clean double. See docs/journal.md.
%  IRREGULARITIES OF THE ORIGINAL, kept: « lernar komplete » set in
%  roman here and in italic at folio 128 for the same example; the
%  German word « aus-sprechen » left in roman where the book everywhere
%  else italicises what is quoted from another language; and, in the
%  note, « transitiva » italic then roman six lines apart.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 137 -- folio 133.
%  THREE INVISIBLE LINES RECOVERED, one of which the assembler had at
%  first wrongly set aside:
%    - « tempi ». » (line 3) and « febla. » (line 29), both announced
%      as « a clean double, no white »;
%    - « arivis. » below the last line of the block of notes. The
%      assembler had declared it a descender after an enlargement whose
%      frame BEGAN AT x = 120 when the page's margin is at x = 71: the
%      word was cut off outside the frame. The surveyor measured it at
%      x 71..160 and read it; checked afterwards under magnification
%      at x 8.
%  A TYPO OF THE ORIGINAL, kept: « dolcacha » for dolcacho, alone of
%  its series in ending in -a.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 138 -- folio 134.
%  IRREGULARITIES OF THE ORIGINAL, kept: « Fundamento » italic on one
%  line and roman six lines below; the same quotation « indikas duro en
%  la ago » italic the first time and roman the second; and two colons
%  in a row in one sentence.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 139 -- folio 135.
%  TWO INVISIBLE LINES RECOVERED, both in the block of notes and both
%  VERY SHORT, which is the detector's known weakness: « ciono e. c. »
%  (y 1573-1588) and « homi? » (y ~1815).
%  The surveyor found them by the INK PROFILE, the second by framing
%  30 px to the left of the margin; the assembler checked them under
%  magnification at x 5.
%  The verdict « a clean double » called the first « with no white »:
%  it was indeed there, but so short that the sum of ink per row falls
%  to 30 px where a full line gives 300.
%  A TYPO OF THE ORIGINAL, kept: in the list of verbs formed directly
%  on the instrument, « martelagar » is printed where the series calls
%  for « martelar » -- and « martelagar » is precisely the form in -ag-
%  that the paragraph opposes to it.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 140 -- folio 136.
%  IRREGULARITIES OF THE ORIGINAL, kept: « meaopinione » run together;
%  « grasajo » given without the equivalence « = ... » that every other
%  member of its series carries; and the same formula set « indikas la
%  objekto » in italic and then « indikas la subjekto » in roman apart
%  from the last word.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 141 -- folio 137.
%  IRREGULARITIES OF THE ORIGINAL, kept: the French title of Couturat's
%  book, in italic elsewhere in the work, is here in ROMAN, its capital
%  E still without an accent; the Esperanto quotations of lines 4 to 8
%  are likewise in roman, where the book everywhere else italicises
%  what is quoted from another language.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 142 -- folio 138. A page with a very long block of notes: 29
%  lines of notes against 14 of body, the rule at 76,88 mm.
%  IRREGULARITIES OF THE ORIGINAL, kept: « -al » in roman in note (1)
%  where the same note italicises it elsewhere; « Geheimer Rat » roman
%  on one line and italic two lines above; « poemi da Vergilius » roman
%  where the parallel examples are in bold; and « oz » without its
%  initial hyphen where the same note writes « -oz » and « -al ».
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 143 -- folio 139.
%  THE NOTE RULE ESCAPED THE TOOL: it is so pale that it becomes
%  continuous only at a threshold of 210, where the detector thresholds
%  at 185. Found by the ink profile at y = 837 px, set on the margin
%  (x = 69) and 210 px = 17,8 mm long -- the 212 px expected. Ordinate
%  laid: 76,12 mm. The detection not having been possible, the surveyor
%  checked the baselines step by step: body 14 lines at a pitch of
%  41,4 px, notes 30 lines at a pitch of 32,7 px, no gap.
%  TYPOS OF THE ORIGINAL, kept: « mam » for kam or nam; « ia vorti »
%  for la vorti; a closing quotation mark with no opening one; and a
%  semicolon where a colon would be expected.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 144 -- folio 140.
%  AN IRREGULARITY OF THE ORIGINAL, kept: « Zamenhof » is set in
%  ordinary roman, where the book's other authors' names are in small
%  capitals; and « ossuaire », « ossuarium », « ossario », « ossuary »
%  stay in roman where « formulary » and « vocabulary », quoted on the
%  same footing two lines above, are in italic.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 145 -- folio 141.
%  TWO INVISIBLE LINES RECOVERED, both VERY SHORT: one in the body
%  (y ~742) and one in the notes (y ~1524). The detection announced
%  them as « a clean double, no white »; the surveyor showed by the ink
%  profile that they carry 536 and 625 px of ink, that is 8 % of a full
%  line -- enough to pass for white in the sum, not enough to be it.
%  The assembler checked them under magnification.
%  TYPOS OF THE ORIGINAL, kept: a white of two ems between « kazi » and
%  its full stop; « L'Espisti » for l'Esperantisti; and « -ul » italic
%  where « -ier », five words further on, is roman.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 146 -- folio 142.
%  IRREGULARITIES OF THE ORIGINAL, kept: « -atr » without its final
%  hyphen where the same suffix is written « -atr- » three times on the
%  page; and, in one paragraph of examples, sponjatra / dolcatra /
%  dolcacha in bold where rezinatra / silexatra / lanatra are roman.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 147 -- folio 143.
%  AN INVISIBLE LINE RECOVERED that the detection had NOT reported:
%  « IV. 521). », the fifth line of note (1). It explains the excess of
%  28 px charged to line 36: it was not a paragraph white but a short
%  line not detected.
%  AN IRREGULARITY OF THE ORIGINAL, kept: « (F. comestibles) » stays in
%  roman where every other foreign quotation on the page is in italic;
%  and « IV. 521 » carries a full stop where the other references to
%  Progreso carry a comma.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 148 -- folio 144.
%  A TYPO OF THE ORIGINAL, kept: « pladego » in the list of examples of
%  the suffix -ed-, where the series calls for « pladedo ».
%  AN IRREGULARITY kept: « boko » is in bold where « pedi », which
%  follows it immediately in the same enumeration, is light.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 149 -- folio 145. The block of notes takes up four fifths of
%  the page: 35 lines of notes against 9 of body, the rule at 59,94 mm.
%  IRREGULARITIES OF THE ORIGINAL, kept: « -eg- » quoted in bold on one
%  line and in italic four lines below; « -ug » and « -et » without a
%  final hyphen where the same forms carry one elsewhere on the page;
%  and the parenthetical glosses of note (3) in roman where the forms
%  they gloss are in italic.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 150 -- folio 146.
%  AN INVISIBLE LINE RECOVERED: « lema e. c. », the last line of the
%  note carried over. The detection was right. The surveyor established
%  it by the ink profile -- a clean cluster of 16 rows carrying 30 px
%  each, that is 8 % of a full line, between two absolute whites --
%  then read it under magnification.
%  IRREGULARITIES OF THE ORIGINAL, kept: « Fundamento » in roman on one
%  line where it is in italic two lines before and three lines after;
%  and « kredemo » in italic where the other occurrences of the same
%  headword are in bold.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 151 -- folio 147.
%  AN INVISIBLE LINE RECOVERED: « defino. », only 117 px wide.
%  A DOUBT RECORDED BY THE SURVEYOR, not settled: the weight of « aj »
%  quoted as a headword. Magnification gives it bold, the width of the
%  stem gives it roman, and the area is washed out by the gutter. Set
%  in bold, by analogy with the page's other quoted suffixes.
%  AN IRREGULARITY OF THE ORIGINAL, kept: « iero » without its initial
%  hyphen where every other member of its list carries one.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 152 -- folio 148.
%  AN IRREGULARITY OF THE ORIGINAL, kept: the same suffix is set in
%  bold in note (1) and in italic in note (3).
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 153 -- folio 149.
%  AN INVISIBLE LINE RECOVERED: « e. c. (1). », 161 px wide.
%  A TYPO OF THE ORIGINAL, kept: « la sufixo es formacas », where the
%  suffix is printed « es » in bare roman where the context calls for
%  the form « -es- ».
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 154 -- folio 150.
%  A TYPO OF THE ORIGINAL, kept: in note (2), « di 1894. en qua il
%  lasis » -- a full stop followed by a lower-case letter, where a
%  comma is expected.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 156 -- folio 152.
%  AN INVISIBLE LINE RECOVERED: « -eri.) », the last line of a
%  paragraph.
%  TYPOS OF THE ORIGINAL, kept: « distinita » for destinita, and
%  « religial » for religiala, on the same line.
%  A DOUBT RECORDED BY THE SURVEYOR: the weight of « direktisto »,
%  which the eye gives roman and the measurement of the stem gives
%  bold, on a line less inked than its neighbours. Set in bold, for
...
% -------------------------------------------------------------------
%  leaf 157 -- folio 153.
%  TYPOS OF THE ORIGINAL, kept: a closing bracket with no opening one
%  (« kun la senco) : »), the bracket opened on the previous line
%  having already been closed; and a break in the italic in mid-phrase,
%  « e quaze » staying in roman between two italic passages.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 158 -- folio 154.
%  AN INVISIBLE LINE RECOVERED in the block of notes: « per --- ». »,
%  which closes the note carried over from the previous folio.
%  TYPOS OF THE ORIGINAL, kept: a colon set tight against the word
%  (« ol nun: ») where the whole page spaces them; and a redundant
...
% -------------------------------------------------------------------
%  leaf 159 -- folio 155.
%  AN IRREGULARITY OF THE ORIGINAL, kept: in the note, « su levas » is
%  in italic and « su kushas » in roman, three words apart.
%  The surveyor also reports that the geometry gives a false paragraph
%  at the penultimate line: it opens on an « = » sign too flat for the
%  glyph filter, and the abscissa is measured on the next word.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 160 -- folio 156.
%  TWO TYPOS OF THE ORIGINAL, kept, and reported by the surveyor as
%  impossible to decide between a compositor's slip and a damaged sort:
%  « scndar » for sendar (an e deprived of its bar) and « Icannes » for
%  Ioannes (an o open on the right). Two accidents of the same kind on
%  the same page make the hypothesis of a fault in the fount plausible;
%  we transcribe what is printed and report it.
% -------------------------------------------------------------------
...
%  leaf 161 -- folio 157.
%  TWO INVISIBLE LINES RECOVERED, both very short: one in the body
%  (~130 px) and one in the notes (~75 px).
%  A TYPO OF THE ORIGINAL, kept: « Dicernez do bone igar do facar. » --
%  the second « do » where the sense calls for « de ». Checked under
%  magnification at x 10.
%  TO BE NOTED: « utensilo » is set with its ending in ITALIC (the
%  suffix quoted within the word itself), and not in bold like the
%  headword that follows two lines below.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 162 -- folio 158.
%  THE RULE WAS LOST BY ONE PIXEL. The surveyor found it by hand at
%  y = 1407 px, then diagnosed the cause in tools/rule.py: the test
%  « nothing else on the rule's line » summed as far as the edge of the
%  image, where the fore-edge of the neighbouring leaf trails. Once
%  corrected (a margin of 25 px taken off on the right), the tool gives
%  exactly the same ordinate as the measurement by hand, and recovers
%  leaf 164 as well.
%  AN IRREGULARITY OF THE ORIGINAL, kept: « (homulo) » in roman and
%  « (homino) » in italic, three words apart; and the same suffixes
%  quoted in italic on one line and in bold on the next.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 163 -- folio 159.
%  A RULE PALER STILL THAN AT LEAF 143: it becomes continuous only at a
%  threshold of 220, where the tool thresholds at 185 and leaf 143 asked
%  for 210. Found at y = 839 px, 216 px long, set on the margin.
%  Ordinate laid: 79,93 mm.
%  TYPOS OF THE ORIGINAL, kept: « iaudigna » for laudigna; a full stop
%  where the sentence calls for a comma; « la spozo » in roman where
%  the word's other occurrences are in italic; and a semicolon before
%  two of the four numbers of an enumeration whose other two carry a
%  colon.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 164 -- folio 160.
%  The rule's ordinate, 124,38 mm, comes from the correction made to
%  tools/rule.py after leaf 162.
%  TWO INVISIBLE LINES RECOVERED by the surveyor, which no detection
%  had reported since the page could not be detected at all: one of
%  85 px in the block of notes, and a last one that fell outside the
%  range the tool explores.
%  AN IRREGULARITY OF THE ORIGINAL, kept: two lists of examples of the
%  same function, one in roman and the other in bold, in the same
%  article.
%  A DOUBT RECORDED: in note (1), the final a of « Kristana » carries a
%  grave accent that Ido does not know and that appears nowhere else;
%  set without the accent, for want of any way of telling a slip from a
%  fault of printing.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 165 -- folio 161. The first page of a gathering: it carries the
%  SIGNATURE « 6 » under the block of text, on the left. Ink surveyed
%  from y 1895 to 1920 px, x 103 to 118.
%  A TYPO OF THE ORIGINAL, kept: a comma between subject and verb in
%  note (5).
%  AN IRREGULARITY kept: in the French gloss, « le fait d'etre mortel, »
%  is in roman where the rest of the parenthesis is italic.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 166 -- folio 162.
%  A DOUBT RECORDED BY THE SURVEYOR, wholesale: the weight of the
%  examples in -izar and -izo. The structure « Ido word = italic gloss »
%  calls for bold everywhere else in the corpus, but direct comparison
%  with a certified bold from leaf 162 gives them roman, and the
%  measurement of the stem does not separate them from the roman beside
%  them. Set in ROMAN, as the facsimile gives them; it is the only
%  point on the folio that could turn over wholesale.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 167 -- folio 163.
%  IRREGULARITIES OF THE ORIGINAL, kept: « kalkoza » in light italic in
%  the middle of an enumeration all of whose other members are in bold;
%  « kontinue » in roman in the middle of a bold example; a full stop
%  where the sentence calls for a comma; and an opening quotation mark
%  never closed.
%  The surveyor reports that tools/weight.py is unusable on this page:
%  the shadow of the binding covers the left third and the tool
%  declares bold words that are plainly roman.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 168 -- folio 164.
%  A TYPO OF THE ORIGINAL, kept: an opening quotation mark never
%  closed. The block of notes carries a single reference and three
%  paragraphs, two of them unnumbered.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 169 -- folio 165.
%  A RULE LOST FOR QUITE ANOTHER REASON THAN THE THRESHOLD. At leaves
%  143 and 163 it was too pale; here it already passes the length test
%  at the ordinary threshold (186 px against 185), and it is the TEST
%  OF THE REMAINDER that rejects it: two specks at x 1301-1335, the
%  fore-edge of the neighbouring leaf, give a remainder of 23 for a
%  threshold of 6. The BORD = 25 px margin laid after leaf 162 is not
%  enough here. Rule found by hand at y = 1791 px, 219 px long,
%  continuous at threshold 220. Ordinate: 157,40 mm.
%  TYPOS OF THE ORIGINAL, kept: « solvor » for solvar; and the same
%  word set roman and then italic on the same line.
%  The surveyor establishes that the list of derivatives in -uro is
%  ROMAN and not bold, against the usage of the chapter: the same stem
%  as the roman words beside it, where the page's headwords are visibly
%  thicker. It is the same doubt leaf 166 left open, settled this time.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 170 -- folio 166.
%  AN IRREGULARITY OF THE ORIGINAL, kept: « sendata » set roman on one
%  line and italic two lines below.
%  The surveyor reports that a diagonal scratch at the edge of the
%  gutter imitates a hyphen of division at the end of a line;
%  checked, the two lines concerned end on a whole word.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 171 -- folio 167.
%  AN INVISIBLE LINE RECOVERED: « tradukir. », the last line of note
%  (1), flush left and very short.
%  TYPOS OF THE ORIGINAL, kept: an « i » deprived of its dot; and a
%  break in the italic in mid-phrase in note (3).
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 172 -- folio 168.
%  THE DETECTOR DID NOT SEE THE RUNNING FOLIO: the first band of the
%  geometry is the centred title « KOMPOZADO. ». The surveyor
%  established it by comparing the page with its neighbours -- line 0
%  at y = 253 and 252 px wide, where the neighbouring leaves have their
%  folio at y 125-153 and 50 to 158 px wide. Checked by
%  tools/apparatus.py, which does see the two distinct lines (folio at
%  y = 143, title at y = 253).
%  AN INVISIBLE LINE RECOVERED: « freno. », the last line of the second
%  paragraph of the note, 90 px long and entirely in italic.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 173 -- folio 169.
%  A RULE TOO PALE, failure (a): it becomes continuous only at
%  threshold 210 where the tool thresholds at 185; at the ordinary
%  threshold the segment starting from the margin is 0 to 1 px and the
%  LENGTH test fails before that of the remainder even runs. The
%  surveyor therefore distinguishes this failure clearly from that of
%  leaf 169. Rule at y = 1303 px, 210 px long.
%  TWO INVISIBLE LINES RECOVERED, absent from the geometry:
%  « horlojo. » (body) and « di vokalo. » (the last line of the notes).
%  A TYPO OF THE ORIGINAL, kept: « l'enfonio » for l'eufonio, the word
%  being correctly set two lines below.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 174 -- folio 170.
%  IRREGULARITIES OF THE ORIGINAL, kept: the gloss of « simio-kapa » is
%  in roman where that of « lenso-forma », one line above and of the
%  same function, is in italic; the same compound is in bold in the
%  body and in italic in the note; two quoted French words stay in
%  roman; and two opening quotation marks are never closed.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 175 -- folio 171.
%  A RULE TOO PALE, failure (a): at the ordinary threshold not one dark
%  pixel in the starting window, the LENGTH test fails before that of
%  the remainder. Continuous at threshold 210. Rule at y = 792 px,
%  209 px long.
%  TYPOS OF THE ORIGINAL, kept: « direte » for direkte; a doubled final
%  stop; « groskapa » without a hyphen in the body where the note
%  writes « gros-kapa »; and a stray full stop in « (5. e 6esma
%  klasi) ».
%  The surveyor notes that the compounds quoted are here in ROMAN where
%  the same ones were in bold at folio 170.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 176 -- folio 172.
%  AN INVISIBLE LINE RECOVERED: « arma. », in italic, flush left,
%  155 px wide, which completed the word broken on the previous line.
%  The second report, « two readings » at line 2, is settled: it is a
%  WHITE, the one that follows the centred title -- the zone is blank
%  under magnification.
%  The surveyor reports a ninth confirmation of the same rule: a list
%  that looked bold on the strip is roman at high magnification.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 177 -- folio 173.
%  A RULE TOO PALE, failure (a): at the ordinary threshold the starting
%  window contains not one dark pixel, the LENGTH test returns n = 0
%  and the rule is set aside before the test of the remainder -- which
%  the surveyor checked in any case: it was 0, there is no speck on the
%  right. Rule at y = 1177 px, 207 px long, continuous at threshold 210.
%  TWO INVISIBLE LINES RECOVERED, both in the notes and both very
%  short: « p. 132. » (85 px) and « tiko. » (53 px).
%  TYPOS OF THE ORIGINAL, kept: « p.497 » with no space after the
%  abbreviating stop, where the same page writes « p. 132. » and
%  « p. 168 »; and a list set in roman where the strictly parallel list
%  of the previous line is in bold.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 178 -- folio 174.
%  TYPOS OF THE ORIGINAL, kept: two opening quotation marks never
%  closed; and the author's initials in ordinary roman capitals where
%  the other authors' names are in small capitals.
%  The surveyor reports the same trap twice, settled under
%  magnification: words that look bold on the strip are roman, and the
%  bold breaks off inside the brackets.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 179 -- folio 175.
%  TWO INVISIBLE LINES RECOVERED, both in the notes: « e. c. (L. B.) »
%  and « kalvini. ».
%  THE SECOND VINDICATES A LESSON OF THE TOOL: the report « ink below
%  the last line » had turned out three times to be foxing; here it is
%  a whole word. The surveyor framed 47 px to the left of the margin
%  and read it without trouble -- exactly the framing the assembler
%  missed at leaf 137, where he took a word for a descender.
%  A PARTICULARITY OF THE ORIGINAL, kept: the abbreviation « DFIS » in
%  full roman capitals, where the other titles quoted are in italic.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 180 -- folio 176. END OF THE SECOND PART.
%  A THIRD DISTINCT CAUSE FOR « RULE NOT FOUND ». It is neither the
%  threshold (143, 163, 173, 175, 177) nor the speck on the right
%  (169): here the rule is at y = 377 px, ABOVE the search window of
%  rule.py, which starts at 0,25 H = 520. The page is almost empty and
%  its block of notes rises very high. The rule passes both tests; it
%  is never examined. Recorded in docs/journal.md.
%  The surveyor also corrects the reading of the page: line 4 is not an
%  indented line of body but THE NOTE itself.
%  The large white before « FINO. » is EMPTY -- no ornament, unlike the
%  end of the first part. Checked by the ink profile at four thresholds
%  and under magnification.
%  A PARTICULARITY OF THE ORIGINAL, kept: no full stop after « ideo »
%  before the author's initials (a clean white of 18 px, read to the
%  pixel).
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaves 181 and 182 -- folios 177 and 178: BLANK.
%  Checked under magnification by the assembler: the few bands the
%  detector surveys there are artefacts of the fold and of the page's
%  edge, none of them is text.
% -------------------------------------------------------------------
\begin{VUpage}[181]{}
\end{VUpage}
...
% -------------------------------------------------------------------
%  leaf 183 -- folio 179. THE APPENDICES OPEN.
%  The page carries NO running folio: the first band is the title. It
%  carries TWO centred lines -- a title in semibold capitals and, below
%  it, a subtitle in lower-case italic in brackets, of a smaller size
%  than the running text (capital height 17 px against 21 for the body).
%  NO NOTE: the surveyor checked the ink profile at thresholds 185, 210
%  and 220 below the last line -- no rule. The appendix pages carry
%  none.
%  AN INVISIBLE LINE RECOVERED: « ad li. » », the end of the first
%  paragraph.
%  The second report, « two readings » at line 2, is settled: it is the
%  white that follows the title block, the zone is blank.
% -------------------------------------------------------------------
\begin{VUpage}[183]{}
...
% -------------------------------------------------------------------
%  leaf 184 -- folio 180. An appendix page, WITH NO NOTES: the surveyor
%  checked the ink profile at thresholds 185, 210 and 220 below the
%  last line -- strictly nil.
%  PARTICULARITIES OF THE ORIGINAL, kept: one word of an accented
%  series left without its accent; the alternation of a circumflex and
%  an acute on the same word two lines apart; and a form carrying both
%  the French orthographic accent and the tonic accent.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 185 -- folio 181. An appendix page, with no notes.
%  AN INVISIBLE LINE RECOVERED: « tionas : », very high in the page,
%  the end of the paragraph begun on the previous folio. The surveyor
%  makes clear that this is NOT the known exception to the verdict « a
%  clean double » (a centred title): there is no title on this folio.
%  AN IRREGULARITY OF THE ORIGINAL, kept: a Greek form in « ph » beside
%  three forms in « f », the passage quoting the Latin spelling.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 186 -- folio 182. An appendix page, with no notes.
%  A PARTICULARITY OF THE ORIGINAL, kept and reported by the surveyor
%  as intended: on the same page, the nominal endings quoted are in
%  BOLD and the verbal endings in ITALIC. Settled at magnification x 8,
%  it is the only point that asked for that magnification.
%  An opening quotation mark is closed nowhere on this folio: the
%  quotation runs on beyond it.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 187 -- folio 183. APPENDIX 2 OPENS.
%  Three elements of apparatus, in this order: an ORNAMENT (a bare rule
%  of 93 px = 7,9 mm, centred), then the title in semibold capitals,
%  then an italic subtitle in brackets, of a smaller size than the
%  running text.
%  THE NOTE RULE: failure (a), too pale. The surveyor established it by
%  replaying the detector -- an isolated speck at x = 95 fixes the
%  start, then nine light pixels the closing does not weld, and the
%  length falls to 1 or 2 px. He rules out (b) by measurement
%  (remainder = 4 and 6, under the threshold) and (c) by position (the
%  rule is indeed inside the window).
%  A PARTICULARITY OF THE ORIGINAL, kept: the letters quoted change
%  their enrichment on the same page -- s in bold three times and then
%  in italic, n in italic and then in bold.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 188 -- folio 184.
%  AN INVISIBLE LINE RECOVERED: « olu. », flush left, the end of the
%  second paragraph of note (2). The verdict « a clean double » keeps
%  its score.
%  TYPOS OF THE ORIGINAL, kept: « niaopinione » run together and
%  « lon » for l'on.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 189 -- folio 185.
%  THE NOTE RULE: failure (a), too pale, established by the same means
%  as at leaf 187 -- the left end of the stroke falls under the
%  threshold, the closing of 7 px does not weld the hole of 12 px, and
%  the length falls back to 1. Rule at y = 1038 px, ordinate 93,98 mm.
%  TYPOS OF THE ORIGINAL, kept: « specala » for speciala;
%  « rispektive » for respektive; and « e. c. e. c. » repeated.
%  A PARTICULARITY kept: in one and the same enumeration of French
%  words, two of them are in italic where all the others are in bold.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 190 -- folio 186.
%  AN ASTERISM, an ornament new to the volume: three asterisks in a
%  triangle between two paragraphs, where the first part made do with a
%  rule. Checked under magnification by the assembler; hence the macro
%  \VUasterismo, added to the preamble.
%  AN INVISIBLE LINE RECOVERED: « à 'tush. », the third line of note
%  (1), only 85 px wide.
%  The surveyor reports that the geometry is wrong on one line: an
%  isolated speck inflates the box and gives a misleading « fin ».
%  A PARTICULARITY OF THE ORIGINAL, kept: the same ending quoted is now
%  in bold, now in roman, five times on the page.
% -------------------------------------------------------------------
...
%  leaf 191 -- folio 187.
%  THE NOTE RULE: failure (a), too pale. The surveyor established it by
%  replaying the detector threshold by threshold -- at 185 and 190 the
%  length is 1 px, at 195 it is 208 and the remainder 0. He rules out
%  (c) by position (the rule is at 0,57 H, the window starts at 0,25 H)
%  and (b) by measurement (remainder = 0). Rule at y = 1184 px,
%  ordinate 105,16 mm.
%  A TYPO OF THE ORIGINAL, kept: « diciplino » for disciplino.
%  The surveyor reports that tools/weight.py is unusable on this page's
%  body: it declares all twenty-two lines BOLD without exception, a
%  uniform verdict and therefore a suspect one. Settled by eye: no bold
%  in the body.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 192 -- folio 188. APPENDIX 3 OPENS.
%  A title in semibold capitals and an italic subtitle in brackets,
%  with no ornament above -- unlike appendix 2, which opened on a rule.
%  Checked: the space between the folio and the title is entirely
%  white.
%  The report « two readings » at line 3 is settled: it is the white
%  that follows the title block, the zone is blank.
%  A PARTICULARITY OF THE ORIGINAL, kept: the same author's name is
%  introduced by « Sioro » written out on one line and by the superior
%  abbreviation eight lines below.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 193 -- folio 189.
%  TYPOS OF THE ORIGINAL, kept: « special » without its -a before a
%  noun, twice; « a ni » and then « ad ni » in two identical turns of
%  phrase; and an opening quotation mark never closed.
%  The surveyor reports two false readings avoided under magnification,
%  both on the same tight ligature.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 194 -- folio 190.
%  THE NOTE RULE: failure (c), too high -- the clearest case in the
%  volume. The rule is at y = 450 px where the search window starts at
%  0,25 H = 516. The surveyor established by measurement that the other
%  two tests pass (length 213 px, remainder 2) and checked it by
%  replaying the detector with the window alone widened: it finds it at
%  once. Ordinate 44,79 mm.
%  AN INVISIBLE LINE RECOVERED: « (frato). », eight characters, absent
%  from the geometry.
%  A TYPO OF THE ORIGINAL, kept: « evas » for esas, checked under
%  magnification at x 6.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 195 -- folio 191.
%  AN INVISIBLE LINE RECOVERED: « p. 37) : », which closes a paragraph
%  of note (3), 180 px wide.
%  PARTICULARITIES OF THE ORIGINAL, kept: a clear white between the
%  bracket and the final stop on the last line of the body; and, in the
%  Esperanto quotation, the digraphs and the grave accent of the old
%  orthography, kept as they stand.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 196 -- folio 192.
%  PARTICULARITIES OF THE ORIGINAL, kept: the same quoted word is in
%  italic and then in roman in the same sentence; and the interrogative
%  particle alternates between two forms on the same page.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 197 -- folio 193. The first page of a gathering: it carries the
%  SIGNATURE « 7 » under the block of notes, on the left. Ink surveyed
%  from y 1882 to 1908 px, x 95 to 111, that is an indent of +38 px --
%  the same as the paragraphs of note.
%  The surveyor reports a reading that shows only under magnification:
%  a compound set HALF ROMAN HALF ITALIC, which at actual size passes
%  for all italic.
%  He also reports that the « ind » of the geometry are misleading on
%  lines opened by an em dash: the detector catches the capital and not
%  the dash.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 198 -- folio 194.
%  AN INVISIBLE LINE RECOVERED, and it is THE FIRST OF THE BODY -- the
%  weightiest case in the volume, since a missing first line shifts the
%  whole page. It is short (162 px), opened by an em dash, and forms a
%  paragraph on its own. No detection had reported it: the surveyor
%  found it because his count of lines would not come out right.
%  Checked under magnification at x 6 by the assembler.
%  A PARTICULARITY OF THE ORIGINAL, kept: a comma where the strictly
%  parallel sentence, four lines above, carries a semicolon.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 199 -- folio 195. APPENDIX 4 OPENS.
%  An ornament (a rule of 96 px = 8,13 mm, centred), a title in
%  semibold capitals, an italic subtitle in brackets.
%  TWO INVISIBLE LINES RECOVERED: « tikala. » (the end of a paragraph,
%  before the ornament) and « homino. » (the end of note 3).
%  The surveyor settled all FOUR reports on the page: line 5 hid both a
%  missing line AND the ornament; line 7 is the white after the title;
%  line 18 is the note rule.
%  PARTICULARITIES OF THE ORIGINAL, kept: a French proverb and a Latin
%  quotation set in ROMAN, where the volume everywhere else italicises
%  what is quoted from another language; and a metathesis in a word
%  repeated correctly on the same line.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 200 -- folio 196.
%  AN INVISIBLE LINE RECOVERED: « vorti : », 110 px wide, the end of a
%  paragraph. It was not the known exception to the verdict « a clean
%  double »: there is no title at that place.
%  The surveyor reports three false readings of weight avoided under
%  magnification, all in the same direction: the strip made roman words
%  look bold.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 201 -- folio 197.
%  The surveyor corrected TWO errors of his own block before handing it
%  in -- a small capital wrongly laid and a group of bold needlessly
%  broken -- rather than let them pass.
%  A TYPO OF THE ORIGINAL, kept: two opening quotation marks in a row,
%  neither of which is closed.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  leaf 202 -- folio 198.
%  A PARTICULARITY OF THE ORIGINAL, kept: on the same page, the same
%  ending quoted is now in bold, now in roman -- the surveyor settled
%  it at magnification 10 by comparing the hairlines.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 208 -- THE ONLY PAGE IN THE VOLUME ENTIRELY AT THE SIZE OF THE
%  NOTES. Note (1), opened at folio 203, overruns and takes the whole
%  of folio 204: forty-three lines. Three measurements show this rather
%  than suppose it -- an x-height of 15-16 px over the forty-six lines
%  (19-20 px at the ordinary size); a pitch of the baselines of
%  33,76 px = 8,13 pt from end to end; and the join in the sense, the
%  last line of folio 203 ending on « ad omna » which the first of 204
%  continues with « ideo abstraktita ».
%  It is that unique pitch that returned « pas -1.00 » to the geometry:
%  the detector was measuring y0, which jumps ten pixels according to
%  the ascenders.
%
%  THERE IS NO RULE. Looked for to the pixel: no ink between y=1688 and
%  y=1735, where leaf 207's rises clean at 213 px. The compositor had
%  nothing from which he had to distinguish his note (*): the page was
%  already entirely at the size of the notes. Hence \VUnotes[0pt].
%  Ordinate: the missing rule would be at y=1685 (1736 less the 51 px
%  that separate rule from first line of note at leaf 207), that is
%  24,30 + (1685-248) x 25,4/300 = 145,97 mm.
%
%  ONE LINE WAS MISSING FROM THE GEOMETRY: « bela. », 65 px wide, under
%  the detector's threshold -- the known failure on very short lines.
%  The true count is 46 lines, not 45.
% -------------------------------------------------------------------
...
\VUnotes{150.88mm}{%
% THE WHITE BETWEEN RULE AND FIRST NOTE IS WIDER HERE than elsewhere
% in the volume: 48 px in the facsimile, where leaf 209 gives 27 and
% \VUsautFilet lays 19. The rule itself is duly in place (y=1744,
% 213 px, checked by eye and by tools/rule.py). We therefore add the
% surplus INSIDE the block, rather than lowering the rule by 20 px to
% set the text: the rule is visible, and we do not move it.
\VUblanc{1.69mm}
...
% -------------------------------------------------------------------
%  LEAF 211 -- THE EXCESS AT INDEX 30 IS NOT THE RULE, IT IS AN
%  ASTERISM. The rule is at y=1626 (211 px), between indices 31 and 32;
%  the block of notes therefore occupies indices 32 to 38, not 30 to 38.
%  The three asterisks in a triangle are at y 1434-1471, checked at x8.
%  Ordinate computed as at leaf 190, whose value \VUasterismo
%  reproduces exactly: 24,30 + (1435-243) x 25,4/300.
%  Note (2) ends on « p. 266, », the bracket unclosed: folio 208
%  carries NO note (checked to the pixel over its whole height, and all
%  its pitches are 41 px), so the note is cut off in the original. Kept
%  as it stands.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 212 -- A PAGE WITH NO NOTE AT ALL. Looked for to the pixel over
%  the whole height, outside the window of tools/rule.py and with the
%  length threshold lowered to 60 px: no candidate. The only long and
%  clean segments are the black edge of the scan, 150 px above the
%  folio. Every pitch of the baselines is 41 to 45 px, never the ~33 px
%  of a block of notes.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 213 -- A FIFTH CAUSE OF « RULE NOT FOUND ». The rule is
%  perfect: y=1372, 214 px, set on the margin, clean to the eye.
%  Ten pixels of dirt at x = 1073-1082, IN THE MIDDLE of the column,
%  carried the « remainder » to 10 for a threshold of 6. The BORD
%  margin, laid for the neighbouring leaf's fore-edge, protects only
%  against the right edge.
%  Fixed in tools/rule.py: we now judge on the size of the largest
%  cluster, not on the total alone.
%  Ordinate: 24,30 + (1372-227) x 25,4/300 = 121,24 mm.
%  The block of notes is ONE note (1) in three paragraphs: the body
%  calls only one « (1) ».
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 214 -- THE GEOMETRY SAYS « excess: none » AND IT IS WRONG:
%  146 px separate index 34 from index 35, and the rule is at y=1698
%  between the two. The excess detector missed the jump; it is the y0
%  that are authoritative.
% -------------------------------------------------------------------
\begin{VUpage}[214]{210}
\VUnotes{148.00mm}{%
% The same wide white as at folio 206: 52 px from the rule to the
% first note, where leaf 209 gives 27. The rule is duly in place
% (y=1698, 212 px); the surplus goes INSIDE the block.
\VUblanc{2.12mm}
...
% -------------------------------------------------------------------
%  LEAF 215 -- AN INVISIBLE LINE RECOVERED: « nitivo. », 106 px wide,
%  between indices 4 and 5 of the geometry. The excess of 41,6 px the
%  detector announced was not a white but a whole leading: the line was
%  there, under the ink threshold. Checked at x7.
%  The body therefore counts 32 lines, not 31.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 216 -- THE VOLUME'S THIRD ASTERISM, after leaves 190 and 211.
%  The detector did not report it: the interval that carries it is
%  exactly four pitches (166,9 px for 4 x 41,40), so that no « excess »
%  came out of it. Measurements: upper row y 1232-1247, lower row
%  y 1254-1269, gap 22 px, lower asterisks 61 px apart, centre x 750
%  where the middle of the measure falls at 749,5. To the pixel, the
%  same values as the other two.
%  Ordinate: 24,30 + (1232-248) x 25,4/300 = 107,61 mm.
%  A PAGE WITH NO NOTE, the volume's second after leaf 212: the three
%  segments a coarse sweep reported (y 639, 764, 1629) are lines of
%  text welded by the morphological closing, 106 to 127 px long where a
%  rule measures 213; the one at y=1629 was read under magnification
%  and carries text.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 217 -- A THIRD PAGE WITH NO NOTE, and one more invisible line:
%  « jugo! », 90 px wide, between indices 30 and 31. It carries only
%  8 % of the ink of a full line -- the known signature. The body
%  counts 41 lines, not 40.
%  No horizontal segment of 90 px or more anywhere on the page: here
%  « rule not found » is the right answer, not a failure.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 218 -- the block of notes takes more than half the page:
%  twenty-two lines in seven paragraphs. The rule, read at y=1142 by
%  the surveyor AND by tools/rule.py, 212 px long: the two readings
%  agree, which was not the case at folio 195. White from rule to first
%  note: 26 px, the ordinary regime.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 219 -- APENDICO 7 OPENS, « VORTORDINO ». A page with no note:
%  the two segments a coarse sweep reported (y 932 and 989, 151 and
%  122 px) are welded lines of text, read under magnification. It is
%  the volume's third page with no note.
%  THE SHORT SECTION RULE ESCAPES THE DETECTOR as it did at leaf 187:
%  91 px wide, 5 px thick, at y 485-489, x 583-674, centred at 628,5
%  for a middle of the measure at 631,5.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 220 -- the volume's fourth page with no note. The four segments
%  of the coarse sweep (y 528, 861, 1540, 1664) were read under
%  magnification: they are welded lines of text.
%  A TYPO OF THE ORIGINAL, kept: « è » for « e » at line 22, a grave
%  accent in metal and not a smudge (solid black, sloping down to the
%  right, checked at x14).
%  A PARTICULARITY kept: at line 9 the counter-example is set in BOLD,
%  where the page's twelve others are in italic.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 221 -- APENDICO 8 OPENS, « PUNTIZADO ». The short section rule
%  escapes the detector again: 100 px wide, 5 px thick, at y 830-834,
%  x 590-689 -- the same starting abscissa as at leaf 187, where its
%  counterpart measures 92 px.
%  The NOTE rule was checked by hand: y=1191, 213 px, set on the margin
%  (x=101 for a margin at 103). The tool's ordinate is confirmed.
%  TYPOS kept: no final stop after « quan ol referas » (the next title
%  opens on a capital all the same); the title « PUNTIZADO » without a
%  final stop, alone in the volume in that; and the italic broken off
%  in the middle of the German quotation of note (2).
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 222 -- THE NOTE RULE IS ATYPICALLY LONG: 226 px where eight
%  witness leaves give 210 to 214. Checked by eye and by hand
%  (y 1828-1830, x 260-505, alone on its row, set on the margin): it is
%  quite real, only 6 % longer. Neither the scan's scale nor the pitch
%  of the lines explains the discrepancy. We keep the volume's standard
%  width, for want of knowing whether the compositor changed his rule
%  or the stroke bled.
%  The white from rule to first note is only 18 px here: the note opens
%  on a bracket, which rises higher than a capital.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 223 -- the volume's fifth page with no note. The three segments
%  of the coarse sweep (y 364, 1525, 1774) were read under
%  magnification: they are runs of letters from the running text.
%  No continuous horizontal run of 60 px anywhere on the page, the edge
%  of the scan apart. A TYPO kept: « separer » for « separar », where
%  the same page gives « separesas » and « separesar ».
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 224 -- TWO PLATES, and THREE ROWS OF DOTS the detector missed.
%  The geometry announces only 34 lines where the page carries 37: the
%  three rows of dots that accompany the vertical brace carry only six
%  dots of 5 px each.
%  The proof is arithmetical: baselines 828,2 — 911 — 952 — 993 —
%  1075,8, that is gaps of exactly 2, 1, 1, 2 pitches.
%
%  THE VERTICAL BRACE is an OPENING one, its point to the left towards
%  the single row: that is what the page states three lines below, « la
%  pinto devas turnesar ad l'unika lineo ». Ink x 786-796, y 877-995.
%
%  THE HORIZONTAL BRACE of the family tree is the volume's only one;
%  hence \VUaccoladeH, added to the preamble. Ink x 524-1063,
%  y 1339-1355, its point at the exact middle.
%
%  TYPOS kept: « (e ne inverse!) Ni konsilas » with no stop after the
%  bracket; « 1) ) » with its white, which is the author's very
%  demonstration; and the welded braces « {} » of line 10, which
%  line 11 nonetheless separates.
% -------------------------------------------------------------------
...
% CENTRED on the measure. Surveyed on scan/r0224.png (block of text
% x 41-1123, measure 1082 px): the diagram's ink runs from 490 to 665,
% that is 449 px on the left (41,5 %) and 458 on the right (42,3 %) --
% a difference of 9 px in 1082. The abscissae below already say so to
% the compositor; the mark says it to the reading page, whose column
% is not the width of the volume.
\VUtabloCentrita
\VUblanc{3.50mm}
...
% CENTRED likewise, and by the same measurement: « Ludovikus » runs
% from 490 to 673 (449 px on the left, 450 on the right) and the
% horizontal brace from 311 to 849 (270 and 274). The tree is
% therefore centred to within one pixel in 1082.
\VUtabloCentrita
...
% -------------------------------------------------------------------
%  LEAF 225 -- the volume's seventh page with no note. The five
%  segments of the coarse sweep all fall three or four pixels below an
%  announced baseline: they are welded serifs.
%  A TYPO kept: the comma after « questionanta » where the sense calls
%  for a full stop (line 37).
%  The facsimile here carries the CROSS as a note reference, with no
%  precedent in the volume.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 226 -- APENDICO 8 ENDS, APENDICO 9 OPENS.
%  The short rule at the end of a section escapes the detector for the
%  third time in the volume: 96 px wide, 5 px thick, at y 1288-1293,
%  x 754-849, centred at 801,5 for a middle of the block at 804,5.
%  It falls between indices 24 and 25, in an interval the list of
%  excesses did not report at all (188,8 px, that is 4,5 pitches).
%  The volume's eighth page with no note: the two segments at
%  y 1764-1765 are the serifs of « komencas per la no... », welded by
%  the coarse threshold three pixels above their baseline.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 227 -- A FOURTH SHORT RULE MISSED BY THE DETECTOR, and a fifth
%  rule, longer, which closes apendico 10.
%  The short one: x 575-673 (99 px), y 766-768, centre 624 for a middle
%  of the block at 626. It lodges in an interval of 178,4 px that the
%  list of excesses did not report at all, as at folio 222.
%  The long one: x 525-726 (202 px), y 1793-1797, centre 625,5, 64 px
%  below the last line. It is TWICE the length of the section rules; it
%  is a stronger division, not a note rule (which is laid flush left,
%  and under which there would be text).
%  The stepped block of indices 2 to 5 is a model address: the right
%  edges do not align (838, 853, 809, 767), so it is not a centred
%  block but four lines of increasing indent.
%  The volume's tenth page with no note.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 228 -- THE LAST PAGE OF THE APENDICI. Seven lines, then the
%  CLOSING ORNAMENT: 296 px = 25,06 mm, the volume's longest rule, at
%  y 1122-1125, centred at 806,5 for a middle of the block at 809,5 --
%  the same 3 px offset as that of folio 222. Its length is that of
%  \VUornementLargeur (25,8 mm), the ornament at the end of a part.
%  Ordinate: 24,30 + (1123-210) x 25,4/300 = 101,60 mm.
%  The volume's eleventh page with no note; a full sweep at three
%  thresholds, nothing on the page but the text and this ornament.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 229 -- THE TABELO OPENS, the analytical index. A new layout: a
%  smaller size (x-height 17 px against 21), leader dots at a pitch of
%  17,25 px, page numbers flush right. Hence \VUpageIndex, \VUindex
%  and \VUindexNu, added to the preamble.
%  THE MEASURE IS THE VOLUME'S: the 976 to 980 px the geometry returns
%  are an artefact of text_region, which cuts off the column of
%  numbers, separated from the dots by a white of 73 px.
%  THE PAGE CARRIES NO FOLIO -- hence the empty argument.
%  AN INVISIBLE LINE RECOVERED: « Pagini. », 88 px wide, under the
%  detector's threshold. The page counts 30 lines, not 29.
%  A CENTRED RULE of 155 px at y 510-513: a fifth length, after 90-100,
%  202 and 296 px.
%  THE NOTE ORDINATE THE TOOL RETURNS (117,35 mm) IS WRONG: it falls in
%  the middle of a line of text. This page has no note at all.
% -------------------------------------------------------------------
\begin{VUpage}[229]{}
...
% -------------------------------------------------------------------
%  LEAF 230 -- the TABELO continues. The page DOES carry a folio,
%  unlike the previous one. The rule detector is wrong a second time
%  running: the 45,81 mm it announces fall on the LEADER DOTS of the
%  entry « an », a horizontal line of ink from x 300 to 1233 which it
%  takes for a rule. No note on this page either.
%  « Prepozicioni : » is the head of a rubric, with its own number and
%  WITHOUT leader dots: it is not a title, and it does not go to
%  \VUtitre.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 231 -- THE RULE DETECTOR'S THIRD LIE ON THE TABELO. It returned
%  74,08 mm; at that height is the BASELINE of the semibold headword
%  « Konjuncioni : », whose morphological closing welds the serifs into
%  a continuous range of 185 px starting from the margin. Two clues
%  were enough to reject it: 185 px instead of the standard 201-246,
%  and the height of a serif.
%  tools/rule.py now narrows its window of length.
%  POSITION DOES NOT SETTLE WEIGHT: six flush-left lines out of ten are
%  here in light roman. We therefore never deduce one from the other.
%  The volume's twelfth page with no note.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 232 -- a page of the TABELO. « Rule not found » is exact here:
%  the only candidate is the weld of serifs of the headword « shamo »,
%  65 px, set on the indent of the sub-entries and not flush left. The
%  volume's thirteenth page with no note.
%  A SECOND EXCESS ESCAPED THE GEOMETRY: the white of +15,2 px that
%  precedes « Nombri : », missed because the detector could not read
%  the baseline of « yen » (bottom = -1,0 in the geometry). The page's
%  two whites are of the same nature and the same size.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 233 -- THE TABELO PASSES TO THE SECOND PART. The centred rule
%  that announces it measures 178 px: a SIXTH length, after 90-100,
%  155, 202 and 296. It is at y 1069-1071, x 533-710, centred at 621,5
%  for a centre of the measure between 618 and 628. The detector missed
%  it as it missed the other five, and it appeared in NO excess: the
%  interval that carries it is 186,5 px, and the list of excesses
%  announced only the two whites of the titles, lower down.
%  The two large excesses of 47 and 44 px are the whites that frame
%  « VORTIFADO »; nothing is hidden there, checked at three thresholds.
%  The number of « indikativo » prints as « 1(6 » -- the zero took the
%  ink on its left half only. Set as 106, which what follows requires.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 234 -- THE GEOMETRY SAW ONLY TWENTY-SIX OF ITS FORTY LINES.
%  The detected block stopped at y = 1300: the last thirteen entries,
%  from « -ad- » to « -em- », fell outside it, and the strips
%  scan/H234_*.png do not even show them. A fourteenth line, « ri- »,
%  was missing from the middle of the block.
%  It is the detector's gravest failure to date: no longer one short
%  line lost, but A THIRD OF THE PAGE. The excess of 52,4 px it
%  announced at index 21 was the sum of a whole line skipped (40 px)
%  and the rubric's white (12 px).
%  The prefixes and suffixes quoted are in ITALIC, not in bold.
%  Typos kept: « anti » and « auto » without a final hyphen where all
%  their neighbours carry one; « sen- » between « ne- » and « par- »,
%  out of alphabetical order; and « -ebl.- » with a full stop before
%  the closing hyphen.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 235 -- FOUR LINES WERE MISSING FROM THE GEOMETRY: « -i- »,
%  « -if- », « -il- » and « -iz- ». The four excesses of a whole
%  leading that it announced correspond to them one for one.
%  THE RULE DETECTOR'S FOURTH LIE ON THE TABELO: it returned
%  122,43 mm, an ordinate that falls INSIDE the bold headword
%  « Kompozado : », whose closing welds the serifs over 181 px.
%  The narrowed window of length now sets it aside, and the detector at
%  last returns « rule not found », which is the right answer.
%  A CENTRED RULE OF 143 px closes the page (x 531-673, y 1835-1837):
%  the volume's seventh length, after 90-100, 155, 178, 202 and 296.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 236 -- THE FLEURON. The volume's only FIGURATIVE ornament: a
%  vegetal vignette, a waving stem carrying leaves, berries and a
%  terminal trefoil (ink x 734-937, y 870-892). It is set with no font
%  of the project; it is CUT FROM THE SCAN and laid as an image. It is
%  the only place where the transcription stops being a composition and
%  becomes a facsimile -- and that must be said.
%  The detector had missed THREE objects: the folio (which it takes
%  everywhere else), the fleuron, and the FULL-MEASURE rule (1084 px)
%  -- too long for its test, which looks for a stroke of 195 to 260 px
%  set on the margin.
%  The interval of 349 px that carries the fleuron appeared in NO
%  excess: the page's only white the list did not report is the one
%  that carried something.
%  TWO PITCHES OF LEADER DOTS on the same page: 17,0 px for the list of
%  apendici, 43,3 px for the two catalogues.
% -------------------------------------------------------------------
...
% -------------------------------------------------------------------
%  LEAF 237 -- MY CONJECTURE ABOUT THE STRUCTURE WAS WRONG. I had
%  announced a publisher's advertisement to the surveyor, going by the
%  structural survey of § 3; the page in fact carries the end of the
%  « Opinioni pri Ido », then the title « Difuzeso di Ido. », then the
%  U. L. I. block. The surveyor measured instead of believing me, and
%  said so.
%  THE LEADING IS TIGHTER THAN ANYWHERE ELSE: 33 px = 7,95 pt between
%  lines of one paragraph, against 41,45 px at the ordinary size. The
%  SIZE itself does not change: an x-height of 16-17 px, that of the
%  running text. The page is set tight, not in a smaller face.
%  No rule, no ornament, no printed folio.
%  Typos kept: « esor » for esar; « Kobenhavn » with a diaeresis where
%  Danish writes Kobenhavn; « Schrijen » for Schrijnen; « College »
%  with a grave accent on an English name.
% -------------------------------------------------------------------
\begin{VUpage}[237]{}
% THE BLOCK OF THIS PAGE RISES HIGHER THAN \VUmargeSup. Its 46 lines
% at a pitch of 33 px, plus the whites of the two titles, ask for
% 1614 px where \VUtexteHauteur offers only 1564: the page overruns by
% 9 mm at the foot if it is set like the others. The volume's last two
% pages are set outside the ordinary frame -- they have no folio
% either, which goes with it.
% For want of a folio to set the page by, the value is taken from the
% measured overrun and not from a reading: it is an adjustment, and
% that must be said.
\vspace*{-9.0mm}
...
% -------------------------------------------------------------------
%  LEAF 238 -- A CATALOGUE OF THE IDIST PRESS AND BOOKSELLERS. No
%  leader dots, no rule, no ornament, no printed folio.
%  THE LEADING IS TIGHT: 32,8 px = 7,90 pt inside a notice, against
%  41,45 px at the ordinary size; between two notices, 15 to 17 px of
%  white more. The « pitch 41,28 px » of the geometry is the gap of one
%  interval only, that of the two centred lines at the head: the true
%  pitch is the opposite.
%  Typos kept: « Ofical » with one f; « Chekoslovakian » without its
%  -a; and « L. de Beaufront » in roman in one notice, « L. DE
%  BEAUFRONT » in small capitals in another.
% -------------------------------------------------------------------
% THE COVER IS NOT SET WIDER THAN THE VOLUME, against what I had
% concluded. The 1172 to 1210 px I had measured came from the GRAIN OF
% THE GREEN PAPER, which a plain thresholding counts as ink: the edge
% of the lines was inflated by a hundred pixels. Taken up again with
% PG.lines_of, which works on a mask of glyphs, this leaf gives a
% maximum width of 1108 px and a median of 1047, for a measure of the
% volume of 1083. The two lines that follow the title therefore have no
% reason to overrun: their letterspacing is set on the 1079 and 1046 px
% measured.
\begin{VUpage}[238]{}
