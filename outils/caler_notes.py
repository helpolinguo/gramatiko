...
outils/filet.py. Le filet est un trait pale de 0,4 pt : selon le seuil et
...
l'ecart du corps une fois retire, se retranche de l'ordonnee.
    ...
    python3 outils/caler_notes.py            # mesure, n'ecrit rien
    python3 outils/caler_notes.py --pose     # ecrit
"""
import os, re, sys
import numpy as np
...
def pose(ecrire=False):
    ...
    for folio, (e, fichier) in sorted(ecarts().items(), key=lambda t: int(t[0])):
        ...
        if abs(e) < SEUIL_PX or not ecrire:
            continue
        fp = os.path.join(P, 'contenu', fichier)
