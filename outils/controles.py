...
A lancer apres CHAQUE lot de modifications :
    ...
    python3 outils/controles.py            # tout
    python3 outils/controles.py 3 4 5      # seulement ces controles
...
def controle_1(pages, r):
    ...
    if hbox:
        r.av('%d « Overfull \\hbox » — inventaire dans outils/hbox.txt'
             % len(hbox))
        with open(os.path.join(P, 'outils', 'hbox.txt'), 'w') as fh:
...
def controle_9(pages, r, seuil=0.12):
    ...
    for i, pg in enumerate(pages, 1):
        ...
        if len(fl) < 6:
            continue
        # Appariement par POSITION, non par rang : voir outils/apparier.py.
...
# outils/page.py, et il se lit de la meme facon : EN AMAS, non en
